...
namespace ne {
    ...
    std::variant<size_t, Error> GlslShaderResourceHelpers::getPushConstantIndex(
        ...
        if (!pMtxPipelineResources->second.pushConstantsData.has_value()) [[unlikely]] {
            return Error("expected push constants to be used");
        }

        auto& pushConstants = pMtxPipelineResources->second.pushConstantsData.value();

        // Make sure that name of this shader resources exists as a field in push constants (in GLSL).
        const auto fieldNameIt = pushConstants.uintFieldIndicesToUse.find(sShaderResourceName);
        if (fieldNameIt == pushConstants.uintFieldIndicesToUse.end()) [[unlikely]] {
            return Error(std::format(
