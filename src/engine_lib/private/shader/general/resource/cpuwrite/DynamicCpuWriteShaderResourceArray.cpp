...
#undef max // <- this macro prevents usage of numeric_limits::max
    ...
    std::optional<Error> DynamicCpuWriteShaderResourceArray::createArray(size_t iCapacity) {
        ...
        // Bind a single resource (not 1 per frame in-flight) even though it's a CPU write resource because we
        // expect the outer code to handle this (the code that uses this array is probably requesting N slots
        // at once - 1 per frame in-flight).
